...
def preprocess_cashflow_data(df: pd.DataFrame, target_company_name: str, access_time: str) -> Dict[str, Any]:
    """현금흐름 데이터를 전처리하여 JSON 형식으로 반환"""
    # 접근 시간 처리
    access_datetime = pd.to_datetime(access_time)
    latest_date = access_datetime - pd.DateOffset(months=1)
    ...
    current_year_data = company_data[(company_data['년'] == latest_year) &
    ...
    for date in reversed(past_12_months):  # 최근 날짜부터 과거 순으로 정렬
        ...
        if not month_data.empty:
            ...
            monthly_data[month_label] = [
                row['현금흐름']['영업활동현금흐름'],
                row['매출액'],
                (row['현금흐름']['영업활동현금흐름'] / row['매출액']
                 * 100) if row['매출액'] else None
            ]
