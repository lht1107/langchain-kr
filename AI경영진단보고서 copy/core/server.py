...
class ServerConfig:
    ...
    def __init__(self):
        ...
        self.log_level = settings.SERVER_LOG_LEVEL.lower()
        self.workers = settings.WORKERS_COUNT
        self.reload = settings.RELOAD_ENABLED
...
def run_server(app: FastAPI) -> None:
    ...
    try:
        ...
        logger.info(f"Log level set to: {settings.SERVER_LOG_LEVEL}")

        if settings.ENV == "development":
            # Development: Uvicorn with reload
            uvicorn.run(
                "main:app",
                host=settings.SERVER_HOST,      # 스킴 없이 호스트명만 전달
                port=settings.SERVER_PORT,
                log_level=settings.SERVER_LOG_LEVEL.lower(),
                reload=settings.RELOAD_ENABLED,
                workers=settings.WORKERS_COUNT if settings.ENV != 'development' else None,
                factory=False
            )
        else:
            ...
            options = {
                ...
                'reload': False,  # Reload disabled in production
