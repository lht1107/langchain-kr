...
class PartnerStabilityMetricsCalculator:
    ...
    def __init__(self, df: pd.DataFrame, target_company_name: str, access_time: pd.Timestamp):
        logger.info(
        ...
        # (self.access_date - pd.DateOffset(months=1)).strftime('%Y-%m')
        self.latest_date = (self.access_date).strftime('%Y-%m')
