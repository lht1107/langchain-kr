...
class FinancialStabilityMetricsCalculator:
    ...
    def __init__(self, df: pd.DataFrame, company_name: str, access_time: pd.Timestamp, n_years: int = 3):
        self.df = self._validate_input_data(df)
        self.company_name = company_name
        self.access_time = access_time  # 이미 pd.Timestamp
    ...
    def _prepare_data(self):
        ...
        # access_time은 최신 날짜(예: 2024-12-30T...)이고, -1개월 전이 실제 데이터 기준
...
def preprocess_financial_stability_data(df: pd.DataFrame, company_name: str, access_time: pd.Timestamp) -> Dict[str, Any]:
    """
    재무 안정성 데이터 전처리 함수
    Args:
        df: 원본 DataFrame
