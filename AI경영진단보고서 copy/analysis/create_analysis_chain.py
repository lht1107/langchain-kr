...
from utils.time_utils import get_access_time  # 기존 임포트 유지
...
async def get_latest_access_time_from_data(data_path: str) -> pd.Timestamp:
    """
    파케이(Parquet) 파일을 읽어, 날짜 컬럼(예: 'updated_at' 또는 'date')의 최대값을 가져온다.
    """
    try:
        df = pd.read_parquet(data_path)
        # 날짜 컬럼 찾기
        date_columns = ['updated_at', 'date', '날짜']
        for col in date_columns:
            if col in df.columns:
                latest_time = pd.to_datetime(df[col]).max()
                logger.debug(
                    f"[Access Time] Latest access time determined from '{col}': {latest_time}")
                return latest_time
        # 날짜 컬럼이 없으면 현재 시간을 반환
        logger.warning(
            "No date columns found in data. Using current timestamp as access_time.")
        return pd.Timestamp.now()
    except Exception as e:
        logger.error(
            f"Failed to determine latest access time from data: {str(e)}")
        raise AnalysisChainError(
            "Failed to determine latest access time from data.") from e
...
async def create_analysis_chain(
    ...
    try:
        ...
        # Step 1.1: Determine access_time dynamically from data
        latest_file_path = os.path.join(settings.DATA_PATH, "sample.parquet")
        access_time = await get_latest_access_time_from_data(latest_file_path)

        if not isinstance(access_time, pd.Timestamp):
            raise ValueError("Invalid access_time extracted from data")
        ...
        try:
            ...
            prompt = PromptTemplate.from_template(
                full_template,
                partial_variables={"latest_year_month": access_time}
            )
        except Exception as e:
        ...
        def process_data(data: pd.DataFrame) -> Dict[str, Any]:
            ...
            try:
                ...
                processed_data = preprocess_func(
                    data, company_name, access_time)  # Use dynamic access_time
