...
class HumanResourcesMetricsCalculator:
    ...
    def __init__(self, df: pd.DataFrame, company_name: str, access_time: str):
    ...
    def calculate_monthly_employee_metrics(self) -> Dict[str, Any]:
        ...
        past_12_months = pd.date_range(
            end=self.latest_date + pd.DateOffset(months=1), periods=12, freq='MS'
        )
        metrics = []
    ...
    try:
        ...
        result = {
            ...
            'average_tenure_by_age': [
                {'연령대': key, '평균근속기간': value} for key, value in hr_dicts['연령별 평균 근속기간'].items()
            ],
            'average_salary_by_age': [
                {'연령대': key, '평균연봉': value} for key, value in hr_dicts['연령별 평균 연봉'].items()
            ],
            'tenure_distribution': [
                {'근속기간': key, '비율': value} for key, value in hr_dicts['근속기간'].items()
            ],
            'age_distribution': [
                {'연령대': key, '비율': value} for key, value in hr_dicts['나이분포도'].items()
            ],
            'monthly_employee_metrics': [
                {key: value for key, value in entry.items()} for entry in monthly_employee_metrics
            ],
            'monthly_salary_and_revenue_metrics': [
                {key: value for key, value in entry.items()} for entry in monthly_salary_and_revenue_metrics
            ]
        }
